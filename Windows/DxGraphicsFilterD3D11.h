// -------------------------------------------------------------------------------
// 
// 		?c?w?ÅÒ?C?u?ÅÒ??		GraphFilter?n?v???O?ÅÒ??( Direct3D11 )?w?b?_?t?@?C??
// 
// 				Ver 3.24f
// 
// -------------------------------------------------------------------------------
...
{
	int						WorkDrawValidGrHandle[ 2 ][ 18 ][ 18 ][ 2 ] ;	// ?t?B???^?[?????????p?`ÅÒ?ÅÒ?Åh\?e?N?X?`??[ 0:???Åh?e?N?X?`?? 1:??Åg????ÅhÅg_?e?N?X?`?? ][ ??( ?Q?????? ) ][ ????( ?Q?????? ) ][ ?????p?Q?Åˆ ]
} ;
...
//extern int	GraphFilter_D3D11_RectBltBase_PF( int IsBlend, int SrcGrHandle, int BlendGrHandle, int DestGrHandle, int BlendRatio, int FilterOrBlendType, int SrcX1, int SrcY1, int SrcX2, int SrcY2, int BlendX, int BlendY, int BlendPosEnable, int DestX, int DestY, va_list ParamList ) ;			// ÅÒ?Åe????t?B???^?[?t??Åg]Åe????s??( ÅÒ??????Åh?????t?? )
...
//extern int	GraphFilter_D3D11_RectBltBase_PF(     GRAPHFILTER_INFO *Info, GRAPHFILTER_PARAM *Param ) ;			// ÅÒ?Åe????t?B???^?[?t??Åg]Åe????s??( ÅÒ??????Åh?????t?? )
