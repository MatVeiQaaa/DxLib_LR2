// -------------------------------------------------------------------------------
// 
// 		?c?w?ÅÒ?C?u?ÅÒ??		GraphFilter?n?v???O?ÅÒ??( Direct3D9 )?w?b?_?t?@?C??
// 
// 				Ver 3.24f
// 
// -------------------------------------------------------------------------------
...
// ?t?B???^?[???Åh?p???ÅÒ?C?u?ÅÒ??????Åf???????????ÅgI??Åe?Åh????????????g???\Åe?Åe?
...
{
	int									WorkDrawValidGrHandle[ 2 ][ 2 ][ 18 ][ 18 ][ 2 ] ;	// ?t?B???^?[?????????p?`ÅÒ?ÅÒ?Åh\?e?N?X?`??[ 0:???Åh?e?N?X?`?? 1:??Åg????ÅhÅg_?e?N?X?`?? ][ ?`???Åg?l???r?b?g?[Ågx  0:16bit  1:32bit ][ ?Q?????? ][ ?????p?Q?Åˆ ]
...
//extern int	GraphFilter_D3D9_RectBltBase_PF( int IsBlend, int SrcGrHandle, int BlendGrHandle, int DestGrHandle, int BlendRatio, int FilterOrBlendType, int SrcX1, int SrcY1, int SrcX2, int SrcY2, int BlendX, int BlendY, int BlendPosEnable, int DestX, int DestY, va_list ParamList ) ;			// ÅÒ?Åe????t?B???^?[?t??Åg]Åe????s??( ÅÒ??????Åh?????t?? )
...
//extern int	GraphFilter_D3D9_RectBltBase_PF(     GRAPHFILTER_INFO *Info, GRAPHFILTER_PARAM *Param ) ;			// ÅÒ?Åe????t?B???^?[?t??Åg]Åe????s??( ÅÒ??????Åh?????t?? )
