// -------------------------------------------------------------------------------
// 
// 		?c?w?ÅÒ?C?u?ÅÒ??		?W???b?ÅÒ?C?u?ÅÒ???g?p?R?[?h		DirectShow BaseClasses
// 
// 				Ver 3.24f
//
...
// Media Foundation ?f?[?^?????p?\Åe?Åe?
typedef struct tagSOUNDCONV_MF
{
	D_IMFSourceReader			*pReader ;						// ?\?[?X???[?_?[
	D_IMFMediaType				*pMediaTypeAudioStream ;		// ????ÅeO???I?[?f?B?I?X?g???[?????f?B?AÅe???
	D_IMFMediaType				*pMediaTypeOutputAudioStream ;	// ?????????I?[?f?B?I?X?g???[?????f?B?AÅe???
	D_SoundConvertMFByteStream	*pByteStream ;					// ?o?C?g?X?g???[??
	LONGLONG					Duration ;						// Åf???
	DWORD						SampleBufferBytes ;				// Åg??????????T?Åg?v??????ÅeÅ˜?????????o?b?t?@???T?C?Y
	void						*pSampleBuffer ;				// Åg??????????T?Åg?v??????ÅeÅ˜?????????o?b?t?@
	DWORD						OutSampleBufferBytes ;			// ?o?????????T?C?Y
	DWORD						UseSampleBufferBytes ;			// ???f?[?^???iÅh[???????????T?C?Y
} SOUNDCONV_MF ;
