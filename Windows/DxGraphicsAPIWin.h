	D_IDXGIAdapter4			*DXGIAdapter4Object ;					// ?c?w?f?h?`???????Åh?Åc?Åf?S?C?Åg?^?[?t?F?C?X
	D_IDXGIFactory			*DXGIFactoryObject ;					// ?c?w?f?h?e?????Åh???Åf???C?Åg?^?[?t?F?C?X
	D_IDXGIFactory1			*DXGIFactory1Object ;					// ?c?w?f?h?e?????Åh???Åf???P?C?Åg?^?[?t?F?C?X
	D_IDXGIFactory2			*DXGIFactory2Object ;					// ?c?w?f?h?e?????Åh???Åf???Q?C?Åg?^?[?t?F?C?X
	D_IDXGIFactory6			*DXGIFactory6Object ;					// ?c?w?f?h?e?????Åh???Åf???U?C?Åg?^?[?t?F?C?X
	D_ID3D11Device			*D3D11DeviceObject ;					// ?c?R?c?P?P?c?Åc???ÅÒ???Åc?C?Åg?^?[?t?F?C?X
	D_ID3D11DeviceContext	*D3D11DeviceContext ;					// ?c?R?c?P?P?c?Åc???ÅÒ???Åc?b?????Åh?Åc???Åh?C?Åg?^?[?t?F?C?X
	D_ID3D11DeviceContext	*D3D11DeferredContext ;					// ?c?R?c?P?P?c?Åc???ÅÒ???Åc?b?????Åh?Åc???Åh?C?Åg?^?[?t?F?C?X( DeferredContext )
	int						D3D11BufferNum ;
...
extern	int		D3D11Device_CheckMultiSampleParam		( D_DXGI_FORMAT Format, UINT *Samples, UINT *Quality, int SamplesFailedBreak ) ;	// ?}???`?T?Åg?v?????Åg?_???Åg?O???T?Åg?v???Åh???N?I???e?B???`?F?b?N
...
extern	int		Direct3D9_CheckMultiSampleParam_ASync				( D_D3DFORMAT Format, D_D3DMULTISAMPLE_TYPE *Samples, DWORD *Quality, int SamplesFailedBreak, int ASyncThread = FALSE ) ;	// ?}???`?T?Åg?v?????Åg?_???Åg?O???T?Åg?v???Åh???N?I???e?B???`?F?b?N
...
extern	int		Direct3DDevice9_CreateVertexBuffer_ASync			( DWORD Length, DWORD Usage, DWORD FVFFlag, D_D3DPOOL Pool, D_IDirect3DVertexBuffer9 **BufferP, int ASyncThread = FALSE ) ;		// Åf?Åg_?o?b?t?@??????????
extern	int		Direct3DDevice9_CreateIndexBuffer_ASync				( DWORD Length, DWORD Usage, D_D3DFORMAT Format, D_D3DPOOL Pool, D_IDirect3DIndexBuffer9 **BufferP, int ASyncThread = FALSE ) ;	// ?C?Åg?f?b?N?X?o?b?t?@??????????
...
extern	int		Direct3D9_CheckMultiSampleParam			( D_D3DFORMAT Format, D_D3DMULTISAMPLE_TYPE *Samples, DWORD *Quality, int SamplesFailedBreak ) ;	// ?}???`?T?Åg?v?????Åg?_???Åg?O???T?Åg?v???Åh???N?I???e?B???`?F?b?N
